...
| D02 | Do not rank or search insertion/deletion candidates in v1. | Structural search is optional wallet behavior beyond BIP93's fixed BCH capacity and has no published ECWs. | Explicit non-goal | Future research may evaluate human-error models outside the reference core |
...
| D08 | Fixed corrections are untrusted suggestions; CLI suggestions use stderr and nonzero status. | BIP93 treats corrections as suggestions. | Accepted | Fixed-correction and CLI contract tests |
