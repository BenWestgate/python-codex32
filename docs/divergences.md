...
| D06 | Default `ms` identifiers encode either 20 public BIP32-fingerprint bits, or 10 fingerprint bits plus the existing non-domain-separated 10-bit share-set tag. | BIP93 leaves identifier selection open. This leaks/link seed metadata and weakens the information-theoretic `<k` disclosure claim. | Medium accepted risk after Gate 3A review | Gate 3; realistic `k-1`-share candidate analysis, frozen fixtures, and metadata-only collision fallback |
...
| D10 | Deriving an additional share rejects the secret index, an existing index, and explicitly excluded indices. | Enforces BIP93's “fresh share index” wording even though the current/Rust reference returns an existing share unchanged. | Implemented | Gate 2 target and exclusion tests |
...
| D14 | Identifier exclusions compare the five-symbol set header (`threshold + identifier`). A colliding derived default changes only metadata; an accepted basis is never discarded. | BIP93 leaves identifier selection/collision policy open. | Accepted and implemented after Gate 3A/3C review | Gate 3 bounded-exclusion validation, payload-preservation tests, and independent diff audit |
